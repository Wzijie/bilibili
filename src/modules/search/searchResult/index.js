    ...
    <ul className='menu-list'>
      {
        searchOrderNavConfig.map((orderNavConfig) => {
          ...
          return (
            <li key={order} className={searchOrder === order ? 'on' : undefined} onClick={onSwitchOrderSearch(order)} >
              <a>{title}</a>
            </li>
          )
        })
          ...
          <ul className='menu-list'>
            {
              searchTypeNavConfig.map((navConfig) => {
                ...
                return (
                  <li key={type} className={searchType === type ? 'menu-active' : undefined} onClick={this.onSwitchTypeSearch(type)} >
                    <a>{title}</a>
                  </li>
                );
              })
